...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	int iFirstRandomNumber, iNextRandomNumber;
	TCHAR str[255];	
	...
	case WM_CREATE:
		...
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Can Not Obtain IRandomNumber Interface"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		}
		...
		pIRandomNumber->GetNextRandomNumber(&iNextRandomNumber);
		
		//show the results
		wsprintf(str, TEXT("The next operator is %2d"), iNextRandomNumber);
		MessageBox(hwnd,str, TEXT("Result"), MB_OK | MB_TOPMOST);

		pIRandomNumber->changeSeed(iNextRandomNumber);
		pIRandomNumber->GetNextRandomNumber(&iNextRandomNumber);

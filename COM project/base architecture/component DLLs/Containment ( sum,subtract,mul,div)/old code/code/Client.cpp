...
LRESULT CALLBACK WndProc(HWND hwnd , UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
		case WM_CREATE:
			...
			//get IMul form server
			pISubtract->QueryInterface(IID_IMultiply, (void**)&pIMul);
			pISubtract->Release();
			pIMul->MultiplicationOfTwoNumbers(n1, n2, &n3);
			wsprintf(str, TEXT("Multiplication of %d and %d is  %d"), n1, n2, n3);
			MessageBox(NULL, str, TEXT("Results"), MB_OK);
			pIMul->QueryInterface(IID_IDivide, (void**)&pIDiv);
			pIMul->Release();
			pIDiv->DivisionOfTwoNumbers(n1, n2, &n3);
			wsprintf(str, TEXT("Div of %d and %d is  %d"),n1,n2,n3);
			MessageBox(NULL, str, TEXT("Results"), MB_OK);
			pIDiv->Release();
			break;
		case WM_DESTROY:
...
void register_server()
{
	...
	wsprintf(gstr, TEXT("%s\\regsvr32.exe /s ContainmentInnerServer.dll"), gszWinSysDir);
	WinExec(gstr,NULL);
	wsprintf(gstr, TEXT("%s\\regsvr32.exe /s ContainmentOuterServer.dll"), gszWinSysDir);
	WinExec(gstr, NULL);
...
{
	GetSystemDirectory(gszWinSysDir, 255);
	wsprintf(gstr, TEXT("%s\\regsvr32.exe /s /u ContainmentInnerServer.dll"),gszWinSysDir);
	WinExec(gstr, NULL);
	wsprintf(gstr, TEXT("%s\\regsvr32.exe /s ContainmentOuterServer.dll"), gszWinSysDir);
	WinExec(gstr, NULL);

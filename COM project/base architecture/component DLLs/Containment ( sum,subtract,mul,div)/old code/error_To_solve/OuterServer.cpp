...
{
	m_pIMultiply->MultiplicationOfTwoNumbers(n1, n2, n3);
}

void __stdcall CSumSubtract::DivisionOfTwoNumbers(int n1, int n2, int *n3)
{
	m_pIDivide->DivisionOfTwoNumbers(n1, n2, n3);
}
...
HRESULT CSumSubtract::InitializeInnerComponent(void)
{
	...
	hr = CoCreateInstance(CLSID_CMultiplyDivide, NULL, CLSCTX_INPROC_SERVER, IID_IMultiply, (void**)&m_pIMultiply);
	if (FAILED(hr))
	{
		return E_NOINTERFACE;
	}

	hr = CoCreateInstance(CLSID_CMultiplyDivide, NULL, CLSCTX_INPROC_SERVER, IID_IDivide, (void**)&m_pIDivide);
	if (FAILED(hr))
	{
		return E_NOINTERFACE;
	}
...
HRESULT __stdcall CFactory::CreateInstance(IUnknown *pUnknownOuter, REFIID iid, void **ppvObject)
{
	...
	//initialize the inner methods
	pCSumSubtract->InitializeInnerComponent();

	//Get the requested interface
	HRESULT hr = pCSumSubtract->QueryInterface(iid, ppvObject);

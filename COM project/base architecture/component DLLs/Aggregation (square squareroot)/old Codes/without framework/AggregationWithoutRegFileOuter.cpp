#include "AggregationWithoutRegFileOuter.h"
#include "CFactory.h"
#include <Windows.h>
#include "Util.h"
...
HRESULT __stdcall CA::QueryInterface(const IID &iid, void **ppv)
{
	...
	{
		//aggregate
		trace("Aggregating to inner!");
		m_pIUnknownInner->QueryInterface(iid, ppv);
	}
	else
	{
		*ppv = NULL;
...
CA::~CA()
{
	...
	if (m_pIUnknownInner)
	{
		trace("Releasin m_pIUnknownInner");
...
HRESULT __stdcall CA::InitializeInnerComponent(void)
{
	HRESULT hr;

	//Check this one
	IUnknown* pUnknownOuter = reinterpret_cast<IUnknown*>(this);

	hr = CoCreateInstance(CLSID_CB,pUnknownOuter,CLSCTX_INPROC_SERVER,IID_IUnknown,(void**)&m_pIUnknownInner);
	if (FAILED(hr))
	{
...
HRESULT __stdcall CFactory::CreateInstance(IUnknown *pUnknownOuter, const IID &iid, void**ppv)
{
	...
	HRESULT hr;
	trace("initializing inner component");
	hr = pCA->InitializeInnerComponent();

	if (FAILED(hr))
	{
		trace("Initialize InnerComponent", hr);
	}

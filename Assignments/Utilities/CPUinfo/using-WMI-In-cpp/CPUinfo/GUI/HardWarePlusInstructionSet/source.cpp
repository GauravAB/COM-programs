...
struct AudioCard
{
	TCHAR* manufacturer[3];
	TCHAR* deviceID[3];
	TCHAR* name[3];
	TCHAR* status[3];
	TCHAR* caption[3];
	TCHAR* description[3];
	TCHAR* errorDescription[3];
	TCHAR* systemName[3];
...
LRESULT CALLBACK WndProc(HWND hwnd , UINT iMsg , WPARAM wParam, LPARAM lParam)
{
	...
	BOOL getAudioInfo(IWbemLocator* pLoc, IWbemServices* pSvc);
	...
	switch (iMsg)
	{
		case WM_CREATE:
			...
			if ( (getAudioInfo(pLoc, pSvc)) == FALSE )
			{
			MessageBox(NULL, TEXT("getAudioinfo failed"), TEXT("failed"), MB_OK);
			DestroyWindow(hwnd);
			exit(0);
			}
...
BOOL getAudioInfo(IWbemLocator* pLoc, IWbemServices* pSvc)
{
	...
	(
		bstr_t("WQL"),
		bstr_t("SELECT * FROM Win32_SoundDevice"), //class from wmi which has all cpu info
		WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY,
		NULL,
		&pEnumerator
	);
	...
	while (pEnumerator)
	{
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.manufacturer[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.manufacturer[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.manufacturer[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.manufacturer[1], _bstr_t(vtProp.bstrVal));
			
			}
			else
			{
				audioinfo.manufacturer[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.manufacturer[2], _bstr_t(vtProp.bstrVal));
			}
		...
		else
		{
			if (uCount == 1)
			{
				audioinfo.manufacturer[0] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.manufacturer[0]," ");

			}
			else if (uCount == 2)
			{
				audioinfo.manufacturer[1] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.manufacturer[1], " ");

			}
			else
			{
				audioinfo.manufacturer[2] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.manufacturer[2], " ");
			}
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.caption[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.caption[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.caption[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.caption[1], _bstr_t(vtProp.bstrVal));

			}
			else
			{
				audioinfo.caption[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.caption[2], _bstr_t(vtProp.bstrVal));
			}
		...
		else
		{
			if (uCount == 1)
			{
				audioinfo.caption[0] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.caption[0], " ");

			}
			else if (uCount == 2)
			{
				audioinfo.caption[1] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.caption[1], " ");

			}
			else
			{
				audioinfo.caption[2] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.caption[2], " ");
			}
		}
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.deviceID[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.deviceID[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.deviceID[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.deviceID[1], _bstr_t(vtProp.bstrVal));

			}
			else
			{
				audioinfo.deviceID[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.deviceID[2], _bstr_t(vtProp.bstrVal));
			}
		...
		else
		{
			if (uCount == 1)
			{
				audioinfo.deviceID[0] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.deviceID[0], " ");

			}
			else if (uCount == 2)
			{
				audioinfo.deviceID[1] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.deviceID[1], " ");

			}
			else
			{
				audioinfo.deviceID[2] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.deviceID[2], " ");
			}
		}
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.description[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.description[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.description[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.description[1], _bstr_t(vtProp.bstrVal));

			}
			else
			{
				audioinfo.description[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.description [2], _bstr_t(vtProp.bstrVal));
			}
		...
		else
		{
			if (uCount == 1)
			{
				audioinfo.description[0] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.description[0], " ");

			}
			else if (uCount == 2)
			{
				audioinfo.description[1] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.description[1], " ");

			}
			else
			{
				audioinfo.description[2] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.description[2], " ");
			}
		}
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.status[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.status[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.status[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.status[1], _bstr_t(vtProp.bstrVal));

			}
			else
			{
				audioinfo.status[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.status[2], _bstr_t(vtProp.bstrVal));
			}
		...
		else
		{
			if (uCount == 1)
			{
				audioinfo.status[0] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.status[0], " ");

			}
			else if (uCount == 2)
			{
				audioinfo.status[1] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.status[1], " ");

			}
			else
			{
				audioinfo.status[2] = (TCHAR*)malloc(sizeof(char));
				strcpy(audioinfo.status[2], " ");
			}
		}
		...
		if (hr == WBEM_S_NO_ERROR && V_VT(&vtProp) != VT_NULL)
		{
			if (uCount == 1)
			{
				audioinfo.name[0] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.name[0], _bstr_t(vtProp.bstrVal));

			}
			else if (uCount == 2)
			{
				audioinfo.name[1] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.name[1], _bstr_t(vtProp.bstrVal));

			}
			else
			{
				audioinfo.name[2] = (TCHAR*)malloc(SysStringByteLen(vtProp.bstrVal));
				strcpy(audioinfo.name[2], _bstr_t(vtProp.bstrVal));
			}
...
void printCPUInfo(HDC hdc)
{
	...
	TextOut(hdc, 200, 460, TEXT("InstructionSet:"), sizeof(TEXT("InstructionSet:")));
	
	
	if (InstructionSet::SSE())
	{
		TextOut(hdc, 230, 480, TEXT("SSE,"), sizeof(TEXT("SSE,")));
	}

	if (InstructionSet::SSE2())
	{
		TextOut(hdc, 270, 480, TEXT("SSE2,"), sizeof(TEXT("SSE2,")));
	}

	if (InstructionSet::SSE3())
	{
		TextOut(hdc, 320, 480, TEXT("SSE3,"), sizeof(TEXT("SSE3,")));
	}

	if (InstructionSet::SSE41())
	{
		TextOut(hdc, 370, 480, TEXT("SSE4.1,"), sizeof(TEXT("SSE4.1,")));
	}

	if (InstructionSet::SSE42())
	{
		TextOut(hdc, 430, 480, TEXT("SSE4.2,"), sizeof(TEXT("SSE4.2,")));
	}

	if (InstructionSet::SSE4a())
	{
		TextOut(hdc, 480, 480, TEXT("SSE4a,"), sizeof(TEXT("SSE4a,")));
	}

	if (InstructionSet::SSSE3())
	{
		TextOut(hdc, 490, 480, TEXT("SSSE3,"), sizeof(TEXT("SSSE3,")));
	}

	if (InstructionSet::MMX())
	{
		TextOut(hdc, 230, 500, TEXT("MMX,"), sizeof(TEXT("MMX,")));
	}

	if (InstructionSet::FMA())
	{
		TextOut(hdc, 270, 500, TEXT("FMA,"), sizeof(TEXT("FMA,")));
	}

	if (InstructionSet::TBM())
	{
		TextOut(hdc, 310, 500, TEXT("TBM,"), sizeof(TEXT("TBM,")));
	}

	if (InstructionSet::ABM())
	{
		TextOut(hdc, 310, 500, TEXT("ABM,"), sizeof(TEXT("ABM,")));
	}

	if (InstructionSet::AES())
	{
		TextOut(hdc, 310, 500, TEXT("AES,"), sizeof(TEXT("AES,")));
	}
	
	if(InstructionSet::AVX())
	{
		TextOut(hdc, 350, 500, TEXT("AVX,"), sizeof(TEXT("AVX,")));
	}

	if (InstructionSet::AVX2())
	{
		TextOut(hdc, 390, 500, TEXT("AVX2,"), sizeof(TEXT("AVX2,")));
	}

	if (InstructionSet::AVX512CD())
	{
		TextOut(hdc, 430, 500, TEXT("AVX512CD,"), sizeof(TEXT("AVX512CD,")));
	}

	if (InstructionSet::AVX512ER())
	{
		TextOut(hdc, 430, 500, TEXT("AVX512ER,"), sizeof(TEXT("AVX512ER,")));
	}


	if (InstructionSet::AVX512F())
	{
		TextOut(hdc, 430, 500, TEXT("AVX512F,"), sizeof(TEXT("AVX512F,")));
	}

	if (InstructionSet::AVX512PF())
	{
		TextOut(hdc, 430, 500, TEXT("AVX512PF,"), sizeof(TEXT("AVX512PF,")));
	}

	if (InstructionSet::HLE())
	{
		TextOut(hdc, 430, 500, TEXT("HLE,"), sizeof(TEXT("HLE,")));
	}

	if (InstructionSet::RTM())
	{
		TextOut(hdc, 470, 500, TEXT("RTM,"), sizeof(TEXT("HLE,")));
	}
...
void printAudioInfo(HDC hdc)
{


	

	TextOut(hdc, 220, 10, TEXT("Audio Card 1") , sizeof(TEXT("Audio Card 1")));

	TextOut(hdc, 220, 250, TEXT("Audio Card 2"), sizeof(TEXT("Audio Card 2")));

	TextOut(hdc, 220, 500, TEXT("Audio Card 3"), sizeof(TEXT("Audio Card 3")));
	
	SetTextColor(hdc, RGB(255, 255, 255));

	

	TextOut(hdc, 0, 30, TEXT("Manufacturer: "), sizeof(TEXT("Manufacturer: ")));
	TextOut(hdc, 160, 30, audioinfo.manufacturer[0], sizeof(TCHAR)*strlen(audioinfo.manufacturer[0]));
	TextOut(hdc, 0, 50, TEXT("Name: "), sizeof(TEXT("Name: ")));
	TextOut(hdc, 160, 50, audioinfo.name[0], sizeof(TCHAR)*strlen(audioinfo.name[0]));
	TextOut(hdc, 0, 90, TEXT("DeviceID: "), sizeof(TEXT("DeviceID: ")));
	TextOut(hdc, 160, 90, audioinfo.deviceID[0], sizeof(TCHAR)*strlen(audioinfo.deviceID[0]));
	TextOut(hdc, 0, 70, TEXT("Status: "), sizeof(TEXT("Status: ")));
	TextOut(hdc, 160, 70, audioinfo.status[0], sizeof(TCHAR)*strlen(audioinfo.status[0]));

	TextOut(hdc, 0, 280, TEXT("Manufacturer: "), sizeof(TEXT("Manufacturer: ")));
	TextOut(hdc, 160, 280, audioinfo.manufacturer[1], sizeof(TCHAR)*strlen(audioinfo.manufacturer[1]));
	TextOut(hdc, 0, 300, TEXT("Name: "), sizeof(TEXT("Name: ")));
	TextOut(hdc, 160, 300, audioinfo.name[1], sizeof(TCHAR)*strlen(audioinfo.name[1]));
	TextOut(hdc, 0, 340, TEXT("DeviceID: "), sizeof(TEXT("DeviceID: ")));
	TextOut(hdc, 160, 340, audioinfo.deviceID[1], sizeof(TCHAR)*strlen(audioinfo.deviceID[1]));
	TextOut(hdc, 0, 320, TEXT("Status: "), sizeof(TEXT("Status: ")));
	TextOut(hdc, 160, 320, audioinfo.status[0], sizeof(TCHAR)*strlen(audioinfo.status[0]));

	TextOut(hdc, 0, 530, TEXT("Manufacturer: "), sizeof(TEXT("Manufacturer: ")));
	TextOut(hdc, 160, 530, audioinfo.manufacturer[2], sizeof(TCHAR)*strlen(audioinfo.manufacturer[2]));
	TextOut(hdc, 0, 550, TEXT("Name: "), sizeof(TEXT("Name: ")));
	TextOut(hdc, 160, 550, audioinfo.name[2], sizeof(TCHAR)*strlen(audioinfo.name[2]));
	TextOut(hdc, 0, 590, TEXT("DeviceID: "), sizeof(TEXT("DeviceID: ")));
	TextOut(hdc, 160, 590, audioinfo.deviceID[2], sizeof(TCHAR)*strlen(audioinfo.deviceID[2]));
	TextOut(hdc, 0, 570, TEXT("Status: "), sizeof(TEXT("Status: ")));
	TextOut(hdc, 160, 570, audioinfo.status[0], sizeof(TCHAR)*strlen(audioinfo.status[0]));
...
void uninitialize(void)
{
	...
	if (gpuinfo.adapterCompatibility[0])
	{
		...
		free(gpuinfo.adapterDACType[0]);
		free(gpuinfo.adapterDACType[1]);
		free(gpuinfo.caption[0]);
		free(gpuinfo.caption[1]);
		free(gpuinfo.Description[0]);
		free(gpuinfo.Description[1]);
		free(gpuinfo.deviceID[0]);
		free(gpuinfo.deviceID[1]);
		free(gpuinfo.driverVersion[0]);
		free(gpuinfo.driverVersion[1]);
		free(gpuinfo.status[0]);
		free(gpuinfo.status[1]);
		free(gpuinfo.videoModeDescription[0]);
		free(gpuinfo.videoModeDescription[1]);
		free(gpuinfo.videoProcessor[0]);
		free(gpuinfo.videoProcessor[1]);
	}
